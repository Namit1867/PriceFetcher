...
contract PriceFetcher is Ownable {
    ...
    /// @notice Use PancakeFactory to find pairs.
    /// @dev Pancake Factory Contract Address.
    ...
    /// @notice WBNB address will be use to calculate price here.
    /// @dev Address of WBNB token..
    /// @return 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
    address public bnbAddress = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; 

    /// @notice BUSD-WBNB(Pancake-LP) address will be use to calculate price here.
    /// @dev Address of BUSD-WBNB token..
    ...
    /// @notice BUSD address will be use to calculate price here.
    /// @dev Address of BUSD token..
    /// @return 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    address public busdAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; 


    /// @notice This mapping will be used for fetching Token Price For Specific Pair Paths
    /// @dev Mapping of token address with their pairPaths.
    /// @return address[]
    mapping(address => address[]) public pricePath;


    /// @notice Set the pricePath Mapping for given (`token`) and (`path`)
    /// @dev Can only be call by the current owner.
    /// @param token address of the token for which path has to be set.
    /// @param path path to set for this token.
    function setPricePath(address token,address[] memory path) external onlyOwner {
        pricePath[token] = path;
    }


    /// @notice Returns current price of given (`token`) .
    /// @param token address of the token for which we have to get price.
    /// @return price price of the token scaled to 18 decimals
    function fetchTokenPrice(address token) external view returns(uint256 price){
        ...
        try ITokenInfo(token).token0() returns(address) {
            ...
            uint factor1 = (token0Price * reserve0 * 1e18) / decimal0;
            uint factor2 = (token1Price * reserve1 * 1e18) / decimal1;
            price = (factor1 + factor2) / totalSupply;

        }
        catch{

            if(pricePath[token].length > 0){
                price = fetchTokenPriceInternal(token, pricePath[token]);
            }
            else{
                ...
                if(tokenWithBnb != address(0)){
                    ...
                    path[0] = tokenWithBnb;
                    path[1] = busdBnbAddress;
                    price = fetchTokenPriceInternal(token, path);
                }
                else if(tokenWithBusd != address(0)){
                    address[] memory path = new address[](1);
                    path[0] = tokenWithBusd;
    ...
    function fetchTokenPriceInternal(address token,address[] memory path) internal view returns(uint256 price) {
    ...
    /// @notice Returns Token Symbol of given token address (`token`) .
    /// @param token address of the token for which we have to get price.
    /// @return symbol symbol of the token in string format
